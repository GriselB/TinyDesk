...
void ReportesUsuarioMenuManager::porcentajeTicketsCompletadosDelUsuario() {
    ...
    if (!sesArch.existeSesion()) {
        cout << "No hay una sesion activa. Inicie sesion para ver sus reportes." << endl;
        pause();
        return;
    }

    if (!sesArch.leer(sesionActual)) {
        cout << "Error al leer la sesion actual." << endl;
        pause();
        return;
    }

    int idUsuarioSel      = sesionActual.getIdUsuario();
    string nombreUsuario  = sesionActual.getNombre();
    string apellidoUsuario = sesionActual.getApellido();

    TicketArchivo ticketArchivo;
    int cantTickets = ticketArchivo.getCantidadRegistros();

    if (cantTickets < 1) {
        cout << "No hay tickets registrados en el sistema." << endl;
        pause();
        return;
    }
    ...
    if (ticketArchivo.leerTodos(vecTickets, cantTickets) < 1) {
    ...
    for (int i = 0; i < cantTickets; i++) {
        ...
        if (t.getIdEmpleado() != idUsuarioSel) continue;

        total++;

        int idEstado = t.getStatus().getIdEstado();

        if (idEstado == 2)      cantFinalizados++;
        else if (idEstado == 0) cantNoDisp++;
        else if (idEstado == 1) cantDisp++;
    }

    delete [] vecTickets;

    if (total == 0) {
        cout << "El usuario " << nombreUsuario << " " << apellidoUsuario
             << " no tiene tickets asignados." << endl;
        pause();
        return;
    }


    int completados = cantFinalizados + cantNoDisp;
    int pendientes  = cantDisp;

    float porcCompletados = (completados * 100.0f) / total;
    float porcPendientes  = (pendientes  * 100.0f) / total;

    SprintManager sprintManager;

    clear();
    cout << "----- PORCENTAJE DE TICKETS COMPLETADOS DEL USUARIO -----" << endl << endl;
    cout << "Usuario: " << nombreUsuario << " " << apellidoUsuario
         << " (ID " << idUsuarioSel << ")" << endl << endl;

    cout << "Tickets totales        : " << total          << endl;
    cout << "Tickets finalizados    : " << cantFinalizados << endl;
    cout << "Tickets no disponibles : " << cantNoDisp      << endl;
    cout << "Tickets disponibles    : " << cantDisp        << endl << endl;

    cout << "Porcentaje completados : " << porcCompletados << "%" << endl;
    cout << "Porcentaje pendientes  : " << porcPendientes  << "%" << endl << endl;

    cout << "Progreso               : "
         << sprintManager.dibujarBarra(porcCompletados) << endl;

    cout << "---------------------------------------------------------" << endl;
    pause();
}

void ReportesUsuarioMenuManager::ticketsProximosAvencerseXusuario() {
    clear();
    Fecha fechaActual;

    TicketArchivo ticketRepo;
    UsuarioArchivo usuarioRepo;
    ProyectoManager proyManager;
    ProyectoArchivo proyRepo;
    SprintArchivo sprintRepo;

    cout << "===== REPORTE DE TICKETS POR VENCER (PROXIMOS 7 DIAS) =====" << endl;

    fechaActual = Fecha("Hoy");
    ...
    for (int i = 0; i < cantTickets; i++) {
        ...
        if (ticketRepo.leer(i, t) == false) {
            continue;
        }


        // Estado == 2 => Finalizado
        if (t.getStatus().getIdEstado() == 2) continue;

        Fecha limite = t.getFechaFin();

        int diasRestantes = fechaActual.diferenciaEnDias(limite);


        //if (diasRestantes < 0 || diasRestantes > 7) continue; // toma solo los que vencen en 7 dias.
        if (diasRestantes > 7) continue; // toma todos los tickets que estan proximos a vencer y los vencidos.

        hayVencimientos = true;


        cout << "---------------------------------------" << endl;
        cout << "Usuario asignado: " << t.getIdEmpleado() << endl;
        cout << "Ticket ID:        " << t.getIdTicket() << endl;
        cout << "Proyecto:         " << t.getNombreProyecto() << endl;
        cout << "Sprint:           " << t.getNombreSprint() << endl;
        cout << "Fecha Limite:     " << limite.toString() << endl;

        if (diasRestantes == 0) cout << "Estado:           VENCE HOY!" << endl;
        else if(diasRestantes < 0) cout << "Estado:           VENCIDO!" << endl;
        else cout << "Dias restantes:   " << diasRestantes << endl;
    }

    if (!hayVencimientos) {
        cout << "---------------------------------------" << endl;
        cout << "Excelente! No hay tickets proximos a vencer en esta semana." << endl;
    }
...
void ReportesUsuarioMenuManager::proyectosParticipaUsuario()
{
    ...
    for(int j= 0;j<cantProy;j++)
    {
        if(vec[j]>0)
        {
            cout<<"El usuario actual esta asignado al proyecto "<<proyectoArchivo.leer(j).getNombre()<<endl<<endl;
            cantidad++;
        }
    }
    if(cantidad==0)
